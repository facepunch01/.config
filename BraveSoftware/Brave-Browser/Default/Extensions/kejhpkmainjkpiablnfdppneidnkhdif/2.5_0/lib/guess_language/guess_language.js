...
(function(global, undefined) {
  ...
  var guessLanguage = function() {
      ...
      if (typeof module === "object" && module.exports === global) {
        models = require('./guess_language_data') || {};
      }
      ...
      function findRuns(text) {
		...
		//console.log(relevant_runs);
        return relevant_runs;
      }

      function identify(text, callback) {

		// Return the matching character percentage against each
		// Unicode character script (set)
      ...
      function check(sample, langs, callback) {
		...
		// Ordered model is a trigram analysis of the selected text
		// Order by trigram frequency
      ...
      function createOrderedModel(content) {
		...
		// Extract trigrams from the selected text
      ...
      function distance(model, known_model) {
		...
		//console.log(known_model);
        for (var i = 0, l = model.length; i < l; i++) {
			
          if (known_model[model[i][0]]) {
			
			// model trigram frequency count - known model 
			//var rank = known_model[model[i][0]];
			//console.log('Trigram ' + model[i][0] + ' Selected text #: ' + model[i][1] + ' | Trigram weight: ' + rank + ' | Distance: ' + Math.abs(model[i][1] - known_model[model[i][0]]));
